    ...
    return(
      <header className={"" + (theme ? "theme " : "") + (headerMargin ? "header-margin " : "header-margin-false ")}>
        ...
        <div className={theme ? "theme-user-login" : "user-login"}>      
          ...
          <div className="login">
            <Link to={loggedIn ? "/userInfo" : "/signup"} >{loggedIn ? currentUser.username : 'Sign Up'}</Link>
            <p className="paragraf">|</p>
            <Link to="/login" onClick = {loggedIn ? this.logout : () => { }}>{loggedIn ? 'Log Out' : 'Log In'}</Link>
          </div>
        </div>
          <nav className={theme ? "theme-nav" : ""}>
            <ul className={toggle ? "toggle" : ""}>
              ...
              }}><i className="fab fa-playstation fa-lg"></i>PSN</Link></li>
              <li><Link to="/xbox" title="XBOX" onClick={()=>{
                setTimeout(() =>{
                  window.location.reload();
                }, 50)
              }}><i className="fab fa-xbox fa-lg"></i>XBOX</Link></li>
        ...
        <div className={toggleLogin ? "toggle-login" : "toggle-login-false"}>
          ...
          <div className={theme ? "theme-profile-side" : "profile-side"}>
            <Link to="/userInfo" className="user-side" title="Profile"><i className="fas fa-user fa-lg"></i>Profile</Link>
